...
<body><nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav mr-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="../reports/financer_report.php">Financer Report</a>
                </li>
                <li class="nav-item">
    ...
    <div class="container">
        <h1 class="mt-5">Financer Report</h1>
        <form action="financer_report.php" method="POST">
            <div class="form-group">
                <label for="financer_id">Select Financer</label>
                <select name="financer_id" class="form-control" required>
                    <?php
                    include('../db.php');
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            echo "<option value='" . $row["financer_id"]. "'>" . $row["name"]. "</option>";
                        }
                    }
        ...
        if (isset($_POST['submit'])) {
            ...
            $sql = "SELECT * FROM asset 
                    INNER JOIN loan ON asset.asset_id = loan.asset_id 
                    INNER JOIN financer_branch ON loan.branch_id = financer_branch.branch_id 
                    INNER JOIN financer ON financer_branch.financer_id = financer.financer_id 
                    WHERE financer.financer_id = $financer_id";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                echo "<h2 class='mt-4'>Report for Financer</h2>
                ...
                echo "<p>No records found for selected Financer</p>";
            }
